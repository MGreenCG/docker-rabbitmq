...
includeFile='/var/lib/rabbitmq/rabbitmq.config.include'
...
if [ "$1" = 'rabbitmq-server' ] && [ "$shouldWriteConfig" ]; then
	...
	# Inject an 'include' config, if present
	if [ -f "$includeFile" ]; then
		includeConfigContent=$(cat "$includeFile")

		fullConfig+=$',\n'
		fullConfig+="$includeConfigContent"
	fi	
